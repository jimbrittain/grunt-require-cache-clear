 ...
 * @param key
 * @return {Boolean}
 * note: done as hasOwnProperty bugged out
 **/
RequireCacheClear.prototype.existsInCache = function(k) {
    if ('hasOwn' in Object && typeof Object.hasOwn === 'function') {
        return Object.hasOwn(require.cache, k);
    } else if ('hasOwnProperty' in require.cache && typeof require.cache.hasOwnProperty === 'function') {
        return require.cache.hasOwnProperty(k);
    } else if ('keys' in Object && typeof Object.keys === 'function') {
        var a = Object.keys(require.cache);
        return (k in a); }
    return (k in require.cache); };

RequireCacheClear.prototype.getCacheLength = function(){
    if(this.doesCacheExist()){
        var count = 0;
        for(var prop in require.cache){
            if (this.existsInCache(prop)) { count += 1; }}
        return count;
    } else { return 0; }};
/**
 * @method run
